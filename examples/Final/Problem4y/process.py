...
def beta_f(SigmaT_h):
    tanh = 0
    for n in range(int(N/2),N):
        tanh = tanh + 2./SigmaT_h * mu[n] * np.tanh(SigmaT_h/2/mu[n]) * w[n]
    return ( 2.0 - tanh ) / ( 1.0 + 3.0 / 4.0 * SigmaT_h**2 * tanh )
'''
def beta_f(SigmaT_h):
    coth = 0
    tanh = 0
    ...
    return ( 2.0 - 2/SigmaT_h * tanh ) / ( 3*SigmaT_h/2 * (tanh+coth) )
'''
# Cases run
I = 16
beta = np.zeros(I)
...
def omega_func_zero(lamb,SigmaT_h):
    ...
    return 1.0 - ( 1.0 + 3.0 / lamb**2 * beta_f_zero(SigmaT_h) ) * omega
